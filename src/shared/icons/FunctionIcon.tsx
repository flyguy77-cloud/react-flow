
export default function FunctionIcon({ className = 'w-6 h-6 text-gray-800' }) {

    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 75 100"
            className={`fill-current ${className}`}
        >
            {/* Witte achtergrondlaag */}
            <rect x="0" y="0" width="75" height="100" rx="12" fill="white" />

            {/* Grijze binnenvlak */}
            <rect x="2" y="3" width="71" height="94" rx="10" className="fill-[#FBE9E7]" />

            {/* Horizontale lijn */}
            <path
                d="M22 50H52"
                stroke="currentColor"
                strokeWidth="6"
                strokeLinecap="round"
                strokeLinejoin="round"
                fill="none"
            />

            {/* Gebogen lijn */}
            <path
                d="M17 77H23.9528C26.2429 76.9949 30.242 75.9738 32 74.5C33.758 73.0262 35.164 71.1925 35.5778 68.9307L39.4222 31.0693C39.836 28.8075 41.242 26.9738 43 25.5C44.758 24.0262 47.7571 23.0051 50.0472 23H57"
                stroke="currentColor"
                strokeWidth="6"
                strokeLinecap="round"
                strokeLinejoin="round"
                fill="none"
            />
        </svg>
    );
}